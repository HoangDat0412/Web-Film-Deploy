<template>
  <div class="container mx-auto p-2">
    <h1 class="text-2xl font-bold mb-4 text-gray-600">Actor Manager</h1>
    ...
    <table class="min-w-full bg-white border">
      ...
      <tbody class="text-gray-600">
        <tr
          v-for="(actor, index) in actorStore.actors"
          :key="index"
          class="border-t"
        >
          <td class="py-2 px-4">
            {{
              (actorStore.currentPage - 1) * actorStore.itemsPerPage + index + 1
            }}
          </td>
          <td class="py-2 px-4">{{ actor.actor_id }}</td>
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    <button
                      ...
                      @click="handleDeleteActor(actor.actor_id)"
                      data-bs-dismiss="modal"
                    >
                      Xóa
                    </button>
      ...
      <ul class="inline-flex -space-x-px text-sm">
        <li>
          <a
            ...
            :class="{
              'bg-gray-100 text-gray-700': actorStore.currentPage === 1,
              'text-gray-500': actorStore.currentPage !== 1,
            }"
        ...
        <li v-for="page in pagesArray" :key="page">
          <a
            ...
            :class="{
              'bg-blue-50 text-blue-600': actorStore.currentPage === page,
              'text-gray-500': actorStore.currentPage !== page,
            }"
        ...
        <li v-if="endPage < actorStore.totalPages">
          <a
            @click.prevent="goToPage(actorStore.totalPages)"
            class="flex text-gray-500 items-center justify-center px-3 h-8 leading-tight border border-gray-300 hover:bg-gray-100 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"
            >{{ actorStore.totalPages }}</a
          >
        </li>
        <li>
          <a
            ...
            :class="{
              'bg-gray-100 text-gray-700':
                actorStore.currentPage === actorStore.totalPages,
              'text-gray-500': actorStore.currentPage !== actorStore.totalPages,
            }"
...
const searchQuery = ref(actorStore.searchQuery);
...
const handleAddNewActor = async () => {
  ...
  await actorStore.addNewActor(data);
...
function handleSearch() {
  actorStore.searchActors(searchQuery.value);
}

const maxPagesToShow = 5; // Maximum number of pages to display

const startPage = computed(() =>
  Math.max(1, actorStore.currentPage - Math.floor(maxPagesToShow / 2))
);
const endPage = computed(() =>
  Math.min(actorStore.totalPages, startPage.value + maxPagesToShow - 1)
);
...
function prevPage() {
  if (actorStore.currentPage > 1) {
    actorStore.goToPage(actorStore.currentPage - 1);
  }
}

function nextPage() {
  if (actorStore.currentPage < actorStore.totalPages) {
    actorStore.goToPage(actorStore.currentPage + 1);
  }
}

function goToPage(page) {
  actorStore.goToPage(page);
}
</script>
